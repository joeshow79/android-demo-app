...
public class PrePostProcessor {
    ...
    private static float mConfThreshold = 0.25f; // score above which a detection is generated
    private static float mIoUThreshold = 0.45f; // score above which a detection is generated
    ...
    static ArrayList<Result> outputsToNMSPredictions(float[] outputs, float imgScaleX, float imgScaleY, float ivScaleX, float ivScaleY, float startX, float startY) {
        ...
        for (int i = 0; i< mOutputRow; i++) {
            if (outputs[i* mOutputColumn +4] > mConfThreshold) {
            //if (outputs[i* mOutputColumn +5] > mConfThreshold) {
                ...
                //Result result = new Result(cls, outputs[i*mOutputColumn+5], rect);
                Result result = new Result(cls, outputs[i*mOutputColumn+4] * outputs[i*mOutputColumn+5], rect);
                results.add(result);
            }
        }
